...
var require_file_command = __commonJS({
  "node_modules/@actions/core/lib/file-command.js"(exports) {
    ...
    function issueFileCommand(command, message) {
      ...
      fs3.appendFileSync(filePath2, `${utils_1.toCommandValue(message)}${os.EOL}`, {
        encoding: "utf8"
      });
    }
...
var changelogType = import_core.default.getInput("changelog_type");
var displayName = import_core.default.getInput("display_name");
var parentFileID = import_core.default.getInput("parent_file_id");
var gameVersions = import_core.default.getInput("game_versions");
var releaseType = import_core.default.getInput("release_type");
var projectRelations = import_core.default.getInput("project_relations");
var filePath = import_core.default.getInput("file_path", { required: true });
var debug = import_core.default.getBooleanInput("debug");
main().catch((err) => {
  console.error(err);
...
async function parseMetadata() {
  if (!parentFileID && !gameVersions) {
    import_core.default.setFailed("You must specify either parent_file_id or game_versions");
    process.exit(1);
  }
  if (parentFileID && gameVersions) {
    import_core.default.setFailed("You cannot specify both parent_file_id and game_versions");
    process.exit(1);
  }
  const metadata = {
    changelog,
...
async function gameVersionsToIds() {
  const availableVersions = await fetch("https://dev.bukkit.org/api/game/versions", {
    ...
    if (!res.ok) {
      ...
      import_core.default.setFailed(`Request failed with status code ${res.status}`);
      process.exit(1);
    }
    return await res.json();
  });
  const idsMap = {};
  availableVersions.forEach((version2) => {
    if (version2.gameVersionTypeID === 1) {
      idsMap[version2.name] = version2.id;
    }
  });
  const parsedGameVersions = [];
  gameVersions.split(", ").forEach((version2) => {
    const id = idsMap[version2];
    if (!id) {
      import_core.default.setFailed(`Invalid game version ${version2}`);
      process.exit(1);
    }
...
async function uploadFile(metadata) {
  ...
  form.append("file", import_fs.default.createReadStream(filePath));
  ...
  await fetch(`https://dev.bukkit.org/api/projects/${projectId}/upload-file`, {
    ...
    import_core.default.setOutput("file_id", json.id);
  });
}
